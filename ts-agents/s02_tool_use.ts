/**
 * s02_tool_use.ts - Tools (TypeScript 版本)
 *
 * 格言: "The loop didn't change at all. I just added tools."
 *
 * s01 的 agent loop 没有改变，我们只是在数组中添加了工具，
 * 并用 dispatch map 来路由调用。
 *
 *     +----------+      +-------+      +------------------+
...
async function main() {
  ...
  while (true) {
    try {
      const query = await askQuestion("\x1b[36ms02 >> \x1b[0m");
